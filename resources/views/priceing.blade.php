        ...
        <div class="row">
            <div class="col-md-12">
                <div class="block text-center">
                    ...
                    <span class="text-white">{{ __('pricing_description') }}</span>
                </div>
            </div>
        </div>
    </div>
...
<section class="section gray-bg" id="pricing">
    <div class="container">
        ...
        <div class="row text-center align-items-end plan_list">
            ...
            @endphp
            <div class="col-lg-4 @if($row->featured == 0) pricing @endif">
                ...
                <div class="bg-white p-5 rounded-lg  @if($row->featured == 1) shadow @endif ">
                    ...
                    <span class="font-weight-bold">@if($row->days == 365) {{ __('Yearly') }} @elseif($row->days == 30)
                        {{ __('Monthly') }} @else {{ $row->days }} {{ __('Days') }} @endif</span>
                    <ul class="list-unstyled my-5 text-small text-left font-weight-normal">
                        <li class="mb-3">
                            {{ __('Products Limit') }} <b>{{ $plan->product_limit == -1 ? __('Unlimited')  : $data->product_limit }}</b>
                        </li>
                        <li class="mb-3">
                            {{ __('Storage Limit') }} <b>{{ $plan->storage == -1 ? __('Unlimited')  : number_format($plan->storage) }}MB</b>
                        </li>
                        <li class="mb-3">
                            {{ __('Use Subdomain') }}
                        </li>
                    ...
                    <a href="{{ route('seller.signup') }}"
                        class="btn site_color text-white btn-block p-2 shadow rounded-pill">{{ __('Register') }}</a>
                    @endif
                </div>
            </div>
            @endforeach
