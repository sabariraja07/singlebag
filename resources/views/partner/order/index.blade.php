				...
				<div class="card-header">
					<div class="col-sm-10">
						<ul class="nav nav-pills">
							<li class="nav-item">
								<a class="nav-link @if($type==='all') active @endif" href="{{ route('partner.order.index') }}">{{ __('All') }}</a>
							</li>

							<li class="nav-item">
								<a href="{{ route('partner.order.index','status=2') }}" class="nav-link @if($type==2) active @endif">{{ __('Pending') }}</a>
							</li>
							<li class="nav-item">
								<a href="{{ route('partner.order.index','status=3') }}" class="nav-link @if($type==3) active @endif">{{ __('Expired') }}</a>
							</li>
							<li class="nav-item">
								<a class="nav-link @if($type===0) active @endif" href="{{ route('partner.order.index','status=cancelled') }}">{{ __('Cancelled') }}</a>
							</li>
					...
					<div class="col-sm-2">
						@can('order.create')
						<a href="{{ route('partner.order.create') }}" class="btn btn-success float-right">{{ __('Create order') }}</a>
					@endcan
				</div>
				--}}
			</div>
					...
					<form>
						...
						<div class="input-group mb-2">
							...
							<select class="form-control selectric" name="term" id="term">
								...
								<option value="email">{{ __('Search By Shop Mail') }}</option>
					...
					<table class="table table-hover table-nowrap card-table text-center">
						...
						<tbody class="list font-size-base rowlink" data-link="row">
							...
							<tr>
								...
								<td class="text-left"><a href="{{ route('partner.order.show_old',$row->id) }}">{{ $row->order_no }}</a></td>
								<td>{{ $row->created_at->format('d-F-Y') }}</td>
								<td><a href="{{ route('partner.shop.show',$row->shop->id) }}">{{ $row->shop->name }}</a></td>
								<td>{{ amount_format($row->amount) }}</td>
								<td>{{ amount_format($row->tax) }}</td>
								<td>{{ $row->PaymentMethod->name ?? 'FREE TRIAL' }}</td>
								<th>{{ amount_format($row->commission) }}</th>
								<td>
								...
								<td>
									<div class="dropdown d-inline">
										...
										<div class="dropdown-menu">
											<a class="dropdown-item has-icon" href="{{ route('partner.order.show_old',$row->id) }}"><i class="far fa-eye"></i> {{ __('View') }}</a>
											<a class="dropdown-item has-icon" href="{{ route('partner.order.invoice',$row->id) }}"><i class="fa fa-file-invoice"></i> {{ __('Download Invoice') }}</a>
										</div>
									</div>
								</td>
							</tr>
