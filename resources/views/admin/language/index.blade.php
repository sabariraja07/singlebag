				...
				<form class="basicform" action="{{ route('admin.languages.active') }}" method="post">
							...
							<table class="table table-hover table-nowrap card-table text-center">
								...
								<tbody class="list font-size-base rowlink" data-link="row">
									...
									<tr>
										...
										<td>{{ $key }}</td>									
										<td>{{ $row }}</td>									
										<td class="text-right"><a href="{{ route('admin.language.show',$key) }}" class="btn btn-success btn-sm"><i class="fa fa-edit"></i></a>
					...
					<div class="form-group row mb-4">
						...
						<div class="col-sm-12 col-md-12">
							<button class="btn btn-success basicbtn" type="submit">{{ __('Save For Main Site') }}</button>
						</div>
					</div>
				</form>
