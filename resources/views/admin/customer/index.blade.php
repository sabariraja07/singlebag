      ...
      <div class="card-body">
        <div class="row mb-2">
          ...
          <div class="col-sm-4 text-right">
            @can('customer.create')
            <a href="{{ route('admin.customer.create') }}" class="btn btn-success">{{ __('Create Customer') }}</a>
            @endcan
          </div>
        </div>
            ...
            <table class="table table-striped table-hover text-center table-borderless">
              ...
              <tbody>
                ...
                <tr id="row{{ $row->id }}">
                  ...
                  <td>{{ storeFolderSize('uploads/'.$row->id) }} / {{ $row->subscription->plan_info->storage == -1 ? __('Unlimited') : human_filesize($row->subscription->plan_info->storage ?? 0) }}</td>
