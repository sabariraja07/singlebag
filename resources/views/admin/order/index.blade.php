				...
				<div class="card-header">
					<div class="col-sm-10">
						<ul class="nav nav-pills">
							...
							<li class="nav-item">
								<a href="{{ route('admin.order.index','status=5') }}" class="nav-link @if($type== 5) active @endif">{{ __('All') }} ({{$all}})</a>
							</li>
							<li class="nav-item">
								<a href="{{ route('admin.order.index','status=1') }}" class="nav-link @if($type==1) active @endif">{{ __('Paid') }} ({{$paid}})</a>
							</li>
							<li class="nav-item">
								<a href="{{ route('admin.order.index','status=2') }}" class="nav-link @if($type==2) active @endif">{{ __('Pending') }} ({{$pending}})</a>
							</li>
							<li class="nav-item">
								<a href="{{ route('admin.order.index','status=3') }}" class="nav-link @if($type==3) active @endif">{{ __('Expired') }} ({{$expired}})</a>
							</li>
							<li class="nav-item">
								<a href="{{ route('admin.order.index','status=0') }}" class="nav-link @if($type==0) active @endif">{{ __('Cancelled') }} ({{$cancelled}})</a>
							</li>
					...
					{{-- <div class="col-sm-2">
						@can('order.create')
						<a href="{{ route('admin.order.create') }}" class="btn btn-success float-right">{{ __('Create order') }}</a>
					@endcan
				</div> --}}
			</div>
		</div>
						...
						<table class="table table-hover table-nowrap card-table text-center">
							...
							<tbody class="list font-size-base rowlink" data-link="row">
								...
								<tr>
									...
									<td class="text-left"><a href="{{ route('admin.order.show',$row->id) }}">{{ $row->order_no }}</a></td>
