			...
			<div class="card-body">
				...
				<form class="basicform" action="{{ route('admin.order.update',$info->id) }}" method="post">
					...
					<div class="form-group row mb-4">
						...
						<div class="col-sm-12 col-md-7">
							<select class="form-control" name="payment_method" disabled>
								...
								<option value="{{ $row->slug }}" @if($info->slug == $row->slug) selected="" @endif>{{ $row->name }}</option>
								@endforeach
							</select>
						</div>
					</div>
					...
					<div class="form-group row mb-4">
						...
						<div class="col-sm-12 col-md-7">
							<input type="text" class="form-control" name="trx" value="{{ $info->trx }}" disabled>
						</div>
					</div>
					<div class="form-group row mb-4">
						<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3" >{{ __('Amount') }}</label>
						<div class="col-sm-12 col-md-7">
							<input type="number" class="form-control" name="amount" value="{{ $info->amount }}">
						</div>
					</div>
					<div class="form-group row mb-4">
						<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3" >{{ __('Discount') }}</label>
						<div class="col-sm-12 col-md-7">
							<input type="number" class="form-control" name="discount" value="{{ $info->discount }}">
						</div>
					</div>
					<div class="form-group row mb-4">
					...
					<div class="form-group row mb-4 " id="email_content">
						...
						<div class="col-sm-12 col-md-7">
							<textarea class="form-control content" id="content" name="content" required></textarea>
						</div>
					</div>
					...
					<small class="text-center">Note: <span class="text-danger">{{ __('If This Order Does Not Used Have Any Payment Gateway The Payment Status Will Not Update') }}</span></small>
				</form>
			</div>
		</div>
	</div>
