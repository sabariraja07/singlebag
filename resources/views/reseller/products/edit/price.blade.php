				...
				<div class="row">
					...
					<div class="col-sm-9">
						...
						<div class="row">
                            ...
                            <label for="price" class="mt-2">{{ __('Supplier Price') }}</label>                         
    
                                <input type="number" disabled value="{{ $info->price->regular_price }}" step="any" class="form-control" id="price" placeholder="Enter Price"  required="">
                            @endif
                            @if(current_shop_type() != 'supplier')
                                ...
                                @if($info->price->price_type === 1 && $info->price->special_price != '')
                                    <span class="badge rounded-pill badge-light-danger" style=" margin-top: 6px; float: right; ">
                                            @if(isset($currency))
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                class="form-control flatpickr-basic" value="{{ $info->price->starting_date }}" />
                        ...
                        <div class="row mb-1">
                            <div class="col-md-12">
                                ...
                                <input type="text" id="special_price_end" name="special_price_end"
                                    class="form-control flatpickr-basic" value="{{ $info->price->ending_date }}" />
                                {{-- <input type="date" class="form-control" id="special_price_end" value="{{ $info->price->ending_date }}" placeholder=""  name="special_price_end" > --}}
                            </div>	
                        </div>			
                        @endif

						<button type="submit" class="btn btn-primary basicbtn mt-2">{{ __('Save Changes') }}</button>	
						</form>
					</div>
				</div>
			</div>
