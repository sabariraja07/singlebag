                        ...
                        @if (current_shop_type() == 'reseller')
                            <div class="col-sm-9">
                                <div class="mb-2 pb-50">
                                    <h5>{{ __('SKU') }} <strong>:</strong></h5>
                                    <span>{{  '--' }}</span>
                                </div>
                                
                                @if ($info->stock->stock_manage == 1)
                                <div class="mb-2 pb-50">
                                    <h5>{{ __('Stock Status') }} <strong>:</strong></h5>
                                ...
                                <div class="mb-2 pb-50">
                                    <h5>{{ __('Stock Quantity') }} <strong>:</strong></h5>
                                    <span>{{ $info->stock->stock_qty ?? '--' }}</span>
                                </div>
                                @else
                                <div class="mb-2 pb-50">
                                    <h5>{{ __('Manage Stock') }} <strong>:</strong></h5>
                                    @if ($info->stock->stock_manage == 1)
                                    <span>  {{ __('Manage Stock') }}</span>
                                    @endif
                                    @if ($info->stock->stock_manage == 0)
                                    <span>{{ __('Dont Need To Manage Stock') }}</span>
                                    @endif
                                </div>
                                @endif
                            </div>
                        @else
                            <div class="col-sm-9">
                                <form class="basicform" method="post"
                                    action="{{ route('seller.products.stock_update', $info->id) }}">
                                    @csrf
                                    <div class="form-group mb-1">
                                        <label for="sku">{{ __('SKU') }}</label>
                                        <input type="text" name="sku" value="{{ $info->stock->sku ?? '' }}"
                                            class="form-control">
                                    </div>
                                    ...
                                    <button type="submit"
                                        class="btn btn-primary basicbtn">{{ __('Save Changes') }}</button>
                                </form>
                            </div>
                        @endif
                    </div>
