        ...
        <div class="content-header row">
            <div class="content-header-left col-md-9 col-12 mb-2">
                <div class="row breadcrumbs-top">
                    <div class="col-12">
                        <h2 class="content-header-title float-start mb-0">{{ __('Edit Variation') }}</h2>
                        <div class="breadcrumb-wrapper">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a href="{{ url('/seller/dashboard') }}">{{ __('Home') }}</a>
                                </li>
                                <li class="breadcrumb-item"><a
                                        href="{{ route('seller.attribute.show', $info->parent_id) }}">{{ __('VARIATIONS') }}</a>
                                </li>
                                <li class="breadcrumb-item active">{{ __('Edit Variation') }}
                                </li>
                            </ol>
                        </div>
                    </div>
                        ...
                        <div class="form-group row mb-4">
                            ...
                            <div class="col-sm-12 col-md-7">
                                <select class="form-control selectric" name="parent_attribute" id="p_id">
                                    @foreach (App\Models\Attribute::where('shop_id', current_shop_id())->whereNull('parent_id')->get() as $row)
                                        <option value="{{ $row->id }}"
                                            @if ($info->parent_id == $row->id) selected="" @endif>{{ $row->name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
