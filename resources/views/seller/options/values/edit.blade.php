        ...
        <div class="content-header row">
            <div class="content-header-left col-md-9 col-12 mb-2">
                <div class="row breadcrumbs-top">
                    <div class="col-12">
                        <h2 class="content-header-title float-start mb-0">{{ __('Edit Option Value') }}</h2>
                        <div class="breadcrumb-wrapper">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a href="{{ url('/seller/dashboard') }}">{{ __('Home') }}</a>
                                </li>
                                <li class="breadcrumb-item"><a
                                        href="{{ route('seller.option.show', $info->product_option_id) }}">{{ __('Option Value') }}</a>
                                </li>
                                <li class="breadcrumb-item active">{{ __('Edit Option Value') }}
                                </li>
                            </ol>
                        </div>
                    </div>
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form class="basicform" action="{{ route('seller.option-value.update', $info->id) }}" method="post">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group row mb-4">
                            <label
                                class="col-form-label text-md-right col-12 col-md-3 col-lg-3">{{ __('Select Attribute') }}</label>
                            <div class="col-sm-12 col-md-7">
                                <select class="form-control selectric" name="product_option_id" id="p_id">
                                    @foreach (App\Models\ProductOption::where('shop_id', current_shop_id())->get() as $row)
                                        <option value="{{ $row->id }}"
                                            @if ($info->product_option_id == $row->id) selected="" @endif>{{ $row->name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
