...
function loadStaticData() {
	...
	$.ajax({
		...
		success: function(response){ 
			...
			$('.product_capacity').html(response.product_limit == -1 ? 'Unlimited' : response.product_limit);
			$('.storage_used').html(response.storage_size);
			$('.storage_capacity').html(response.storage  == -1 ? 'Unlimited' : response.storage);
			$('.posts_used').html(response.products + ' / ' + (response.product_limit == -1 ? 'Unlimited' : response.product_limit));
			$('.plan_load').hide();
			$('.product_used').hide();
			...
			pie_storage(response.storage_used,response.storage_pie);
		...
		options: {
			...
			scales: {
				yAxes: [{
					...
					ticks: {
						// stepSize: Math.round((getArrSum(visitors) + getArrSum(pageViews)) / 10),
					}
				}],
				xAxes: [{
					gridLines: {
...
function pie_storage(a,b) {
	...
	if(b == -1){
		$(".sparkline-pie-storage").html(`<div role="progressbar" aria-valuenow="100" aria-valuemin="0" aria-valuemax="100"
		style="--value: 100;margin: auto;"></div>`)
	}else{
...
function pie_product(a,b) {
	...
	//b limit
	if(b == -1){
		$(".sparkline-pie-product").html(`<div role="progressbar" aria-valuenow="100" aria-valuemin="0" aria-valuemax="100"
		style="--value: 100;margin: auto;"></div>`)
	}else{
...
function load_perfomace_chart(dates,totals) {
	...
	//  ticks: {
	// 	beginAtZero: true,
	// 	// stepSize: Math.round(getArrSum(totals)/ 10),
