	...
	$.ajax({
		...
		success: function(response){ 
			...
			$.each(response, function(index, row){
				var html='<div class="form-check form-check-primary mb-2 shipping_method"><input id="'+row.id+'" name="shipping_method" data-amount="'+row.slug+'" value="'+row.id+'" type="radio" class="cform-check-input method'+row.id+'" onclick="calculateShipping('+row.id+')" required><label  class="custom-control-label" for="'+row.id+'">'+(row.name)+'('+(row.parent_estimate_relation[0]?.content ?? '' )+')</label></div>';
