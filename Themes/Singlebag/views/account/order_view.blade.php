...
@section('account_breadcrumb')
    <span></span> {{ __('Order Information') }}
@endsection
                                    ...
                                    @foreach ($info->order_item as $row)
                                        <tr>
                                            <td>
                                                ...
                                                @else
                                                    <a class="unpublished"
                                                        @click="Sweet('error','This Product is no longer available in this site.')">{{ Str::limit($row->term->title, 50) ?? '' }}</a>
                                                @endif
                                                <br>
                                                @php
                                                    $attributes = $row->info;
                            ...
                            <div class="col-lg-4 text-right">
                                <div class="invoice-detail-item">
                                    <div class="invoice-detail-name">
                                        <strong>{{ __('Subtotal') }}:</strong>{{ amount_format($order_content->sub_total ?? 0 + $order_content->coupon_discount ?? 0) }}
                                    </div>
                                </div>
                                <div class="invoice-detail-item">
                                    <div class="invoice-detail-name">
                                        <strong>{{ __('Discount') }}:</strong>{{ amount_format($order_content->coupon_discount ?? 0) }}
                                    </div>
                                </div>

                                <div class="invoice-detail-item">
                                    <div class="invoice-detail-name"><strong>{{ __('Tax') }}:</strong>
                                        {{ amount_format($info->tax) }}</div>
                                </div>
                                <div class="invoice-detail-item">
                                    <div class="invoice-detail-name"><strong>{{ __('Shipping') }}:</strong>
                                        {{ amount_format($info->shipping) }}</div>
                                </div>
                                <hr class="mt-2 mb-2">
                                <div class="invoice-detail-item">
                                    <div class="invoice-detail-name">{{ __('Total') }}</div>
                                    <div class="invoice-detail-value invoice-detail-value-lg">
                                        {{ amount_format($info->total) }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
