<template>
    <div class="search-style-2 header-search">
        <form action="javascript:void(0);">
            <select class="select-active category-menu" id="search-category-id" v-select="filter.category_id" v-model="filter.category_id" @change="get_shop_products()">
                        ...
                        <li class="list-item"  v-for="item in suggestions" :key="item.id">
                            <a :href="'product/' + item.slug + '/' + item.id"
                                ...
                                <div class="product-image">
                                    <img :src="item.image" @error="$event.target.src='uploads/default.png'" alt="">
                                </div>
