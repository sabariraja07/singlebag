...
class Helper
{
	...
	public static function domain($domain,$full_domain)
	{
		...
		if (!Cache::has(Helper::$domain)) {
			...
			$value = Cache::remember(Helper::$domain, 300,function () {
				$data=\App\Models\Domain::where('domain',Helper::$domain)->where('status',1)->with('theme')->first();
				if (empty($data)) {
					die();
				}
	...
	public static function autoload_site_data(){
		if(!Cache::has(domain_info('shop_id').'autoload_loaded')){
		$autoload_data=\App\Models\ShopOption::where('shop_id',domain_info('shop_id'))->where('status',1)->where('key','!=','currency')->get();

		if(count($autoload_data) > 0){
			Cache::remember(domain_info('shop_id').'autoload_loaded',300,function(){
				return true;
			});
		}
		foreach($autoload_data as $autoload){
			if($autoload->key == 'local'  && Session::has('locale_changed') == false){
				Session::put('locale',$autoload->value);
			}
			else{
				Helper::$autoload_static_data=$autoload->value;
				Cache::remember(domain_info('shop_id').$autoload->key, 300,function () {
	...
	public static function autoload_main_site_data(){
		...
		if(!Cache::has('marketing_tool')){
			$marketing_tool=\App\Models\Option::where('key','marketing_tool')->first();
		...
		if(!Cache::has('active_languages')){
			$marketing_tool=\App\Models\Option::where('key','active_languages')->first();
			if(!empty($marketing_tool)){
				Helper::$autoload_static_data=json_decode($marketing_tool->value);
	...
	public static function menu_query($menu_position){
		...
		return $menus=cache()->remember($menu_position.'menu'.domain_info('shop_id'), 300, function () {
			$shop_id=domain_info('shop_id');
			$menus=Menu::where('position',Helper::$position)->where('shop_id',$shop_id)->first();
