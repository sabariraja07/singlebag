...
class CronController extends Controller
{
    ...
    public function makeExpireAbleCustomer()
    {
        ...
        Subscription::where('will_expire', '<', Carbon::today())->where('status', 1)->update(array('status' => 3)); //expired
    ...
    public function send_mail_to_will_expire_plan_soon()
    {
        ...
        $tenants = \App\Models\Shop::where([['status', 1], ['will_expire', '<', $date], ['will_expire', '!=', Carbon::now()->format('Y-m-d')]])->with('SubscriptionWithPlan', 'user')->get();


        $expireable_tenants = [];

        foreach ($tenants as $row) {
            $plan = $row->SubscriptionWithPlan->plan;
